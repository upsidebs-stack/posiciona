...
- **2026-09-01** — Dono passou o `DATABASE_URL` real do Neon. Rodei `drizzle-kit migrate` contra ele: **`migrations applied successfully!`**. Conferi direto no banco via `information_schema.tables` — as 22 tabelas e as extensions `citext`/`pgcrypto` estão todas lá. **Critério "migration em banco limpo" da Fase 0: cumprido, rodando de verdade.** `DATABASE_URL` configurada no Netlify também; redeploy em andamento para o site em produção passar a usar o banco real.
- **2026-09-01** — Testei o link mágico em produção de verdade e bati em dois problemas reais, os dois só aparecem fora do localhost:
  1. **UntrustedHost do Auth.js.** Em produção, o Auth.js recusa qualquer Host que não reconheça, por segurança. Faltava AUTH_TRUST_HOST=true — variável que não estava no PLANO.md seção 17 porque só importa fora do localhost (o dev sempre confia). Adicionada em .env, .env.example e no Netlify.
  2. **RESEND_API_KEY x AUTH_RESEND_KEY.** O provider Resend do Auth.js só lê a chave sozinho de uma variável chamada AUTH_RESEND_KEY — como o PLANO.md já batizou a variável de RESEND_API_KEY, o provider simplesmente não achava a chave e mandava "Bearer undefined" pro Resend, que respondia "API key is invalid" (confundi por um instante com a chave estar errada — não estava; testei ela direto contra a API do Resend e voltou 200). Corrigido passando apiKey: process.env.RESEND_API_KEY explicitamente em src/auth.ts, mantendo o nome de variável do plano.
  - Depois dos dois ajustes: refiz o deploy e testei o formulário de novo em produção.
- **2026-09-01** — Testei o formulário de novo em produção depois dos dois ajustes: a tela foi para "Check your email" (não mais erro). Confirmei a entrega de verdade consultando a API do Resend (`GET /emails`) — o evento mais recente mostra `"last_event": "delivered"` para `upsidebs@gmail.com`. **Os três critérios de aceite da Fase 0 estão cumpridos e comprovados: deploy verde (site no ar, CI verde no GitHub), migration em banco limpo (22 tabelas + extensions confirmadas no Neon), e magic link funcionando (e-mail de fato entregue, não só a tela mudando).**
- **2026-09-01** — Dono clicou no link recebido de verdade e confirmou: caiu na tela "Your businesses / Signed in as upsidebs@gmail.com". **Fase 0 fechada e confirmada pelas duas pontas — pelo Resend (entrega) e pelo próprio dono (login funcionando na tela).**

## Fase 1 — em andamento

- **2026-09-01** — Copiei `attributes.ts`, `gaps.ts` e `rubric.md` do `seed/src/domain/` para `src/domain/` (o app de verdade). Escrevi os testes de `gaps.ts` primeiro, como o briefing pediu: 19 casos cobrindo os quatro cenários que o PLANO.md cita (mercado uniforme, sábado já coberto, conjunto pequeno demais, maioria sem horário) mais SERVICE_TYPE, PRICE_BAND, DIGITAL, SPEED, `topGaps` e `gapsForAxes`. Todos passando.
- Implementei `src/domain/positioning.ts` (seção 8.1 e 6.4 do PLANO.md: `attrDistance`, `densityGrid`, `relevance`, `rivalry`) com 17 testes cobrindo as bordas que a seção 15 pede — scores iguais, 1 concorrente, 0 concorrentes, franquia esgotada não vira negativo.
- Implementei `src/domain/schemas.ts` — os contratos Zod da seção 7.2, fiéis ao texto do plano. 9 testes de fixture confirmando que evidência vazia é rejeitada e que `gap_ids` vazio derruba uma `position_option`.
- Implementei `src/domain/gating.ts` (seção 9) — `getTier` e `serializeAnalysis`. **O fuzz test obrigatório existe e passa**: 200 análises sintéticas determinísticas, cada uma checando que nenhum nome de concorrente oculto, nenhuma `structural_gap`, e nenhum campo bloqueado sobrevive na serialização FREE, em nenhuma profundidade do JSON.
  - Adicionei um `structuralGapCount` na resposta FREE — a seção 9 do plano pede mostrar "a contagem" das lacunas mas o trecho de código só mostrava `hiddenCompetitorCount`; era uma lacuna do exemplo, não uma escolha de produto.
- Corrigi de passagem: o `eslint.config.mjs` estava enxergando `.netlify/` (pasta de build local) e gerando milhares de avisos de ruído; adicionei ao ignore. Também liguei `ignoreRestSiblings` no `no-unused-vars`, porque o padrão `const { segredo, ...resto } = x` de `gating.ts` é legítimo e o eslint padrão o marca como erro.
- **47 testes passando** (`npm run test`), typecheck e lint limpos.
